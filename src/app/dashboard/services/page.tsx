        ...
        <div className="flex gap-1 bg-slate-100 rounded-lg p-1">
          {(['all', 'nettoyage', '3d'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 text-sm rounded-md transition ${filter === f ? 'bg-white shadow text-slate-900 font-medium' : 'text-slate-500 hover:text-slate-700'}`}>
            ...
            <div key={service.id}
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${service.domaine === 'nettoyage' ? 'bg-blue-100' : 'bg-amber-100'}`}>
                    {service.domaine === 'nettoyage' ? <Sparkles className="w-5 h-5 text-blue-600" /> : <Bug className="w-5 h-5 text-amber-600" />}
                  </div>
                  <div>
                    <h3 className="font-semibold text-slate-900">{service.nom}</h3>
                    {service.description && <p className="text-xs text-slate-500 mt-0.5">{service.description}</p>}
                    <div className="flex items-center gap-2 mt-2">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${service.domaine === 'nettoyage' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>
